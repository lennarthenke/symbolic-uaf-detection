...
class GetenvHook(angr.SimProcedure):
    """
    The getenv() function searches the environment list to find the
    environment variable name, and returns a pointer to the
    corresponding value string.

    EDIT: Now supports uninitialized BVS
    """

    def run(self, m_name: BV):
        ...
        if p is None:
            return self.state.solver.BVS(b"getenv__" + name, self.state.arch.bits, key=("api", "getenv", name.decode()), uninitialized=True)
